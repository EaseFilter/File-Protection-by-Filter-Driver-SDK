...
namespace EaseFilter.CommonObjects
{
    ...
    public class GlobalConfig
    {
        ...
        static string accountName = "support@easefilter.com";
        ...
        static string drInfoFolder = "c:\\DRInfoFolder";
        static string shareFolder = "";
        ...
        {
            if (processFilterRules.ContainsKey(filterRule.ProcessId + filterRule.ProcessNameFilterMask))
            {
                processFilterRules.Remove(filterRule.ProcessId + filterRule.ProcessNameFilterMask);
                ConfigSetting.RemoveProcessFilterRule(filterRule);
            }

            processFilterRules.Add(filterRule.ProcessId + filterRule.ProcessNameFilterMask, filterRule);
        ...
        {
            if (processFilterRules.ContainsKey(processId + processNameFilterMask))
            {
                return processFilterRules[processId + processNameFilterMask];
            }
        ...
        public static bool AddRegistryFilterRule(RegistryFilterRule newRule)
        {
            if (registryFilterRules.ContainsKey(newRule.ProcessId + newRule.ProcessNameFilterMask))
            {
                registryFilterRules.Remove(newRule.ProcessId + newRule.ProcessNameFilterMask);
                ConfigSetting.RemoveRegistryFilterRule(newRule.ProcessId, newRule.ProcessNameFilterMask);
            }

            registryFilterRules.Add(newRule.ProcessId + newRule.ProcessNameFilterMask, newRule);
            ConfigSetting.AddRegistryFilterRule(newRule);

            return true;
        }

        public static void RemoveRegistryFilterRule(RegistryFilterRule registryFilterRule)
        {
            if (registryFilterRules.ContainsKey(registryFilterRule.ProcessId + registryFilterRule.ProcessNameFilterMask))
            {
                registryFilterRules.Remove(registryFilterRule.ProcessId + registryFilterRule.ProcessNameFilterMask);
                ConfigSetting.RemoveRegistryFilterRule(registryFilterRule.ProcessId,registryFilterRule.ProcessNameFilterMask);
            }
        }

        public static RegistryFilterRule GetRegistryFilterRule(string processId, string processNameFilterMask)
        {
            if (registryFilterRules.ContainsKey(processId + processNameFilterMask))
            {
                return registryFilterRules[processId + processNameFilterMask];
            }
            ...
            try
            {
                ...
                foreach (ProcessFilterRule filterRule in processFilterRules.Values)
                {
                    if (filterRule.ControlFlag > 0 && filterRule.ProcessNameFilterMask.Length > 0)
                    {
                        ...
                        if (!FilterAPI.AddProcessFilterEntry((uint)filterRule.ProcessNameFilterMask.Length * 2, filterRule.ProcessNameFilterMask, filterRule.ControlFlag))
                        {
                            EventManager.WriteMessage(791, "AddProcessFilterEntry", EventLevel.Error, "AddProcessFilterEntry " + filterRule.ProcessNameFilterMask + " failed:" + FilterAPI.GetLastErrorMessage());
                        }
                        else
                        {
                            EventManager.WriteMessage(791, "AddProcessFilterEntry", EventLevel.Information, "AddProcessFilterEntry " + filterRule.ProcessNameFilterMask + ",ProcessControlFlag:" + filterRule.ControlFlag + " succeeded.");
                        }
                        ...
                        foreach (string fileAccessRight in fileAccessRights)
                        {
                            if (fileAccessRight.Trim().Length > 0)
                            {
                                ...
                                if (filterRule.ProcessId.Length > 0)
                                {
                                    ...
                                    for (int i = 0; i < pids.Length; i++)
                                    {
                                        ...
                                        if (uint.TryParse(pids[i], out processId) && processId > 0)
                                        {
                                            ...
                                            if (!FilterAPI.AddFileAccessRightsToProcessId(processId, (uint)fileNameMask.Length * 2, fileNameMask.Trim(), accessFlag))
                                            {
                                                EventManager.WriteMessage(725, "AddFileAccessRightsToProcessName", EventLevel.Error, "AddFileAccessRightsToProcessName " + filterRule.ProcessNameFilterMask + ",fileNameMask:" + fileNameMask + ",accessFlags:" + accessFlag + " failed:" + FilterAPI.GetLastErrorMessage());
                                            }
                                            else
                                            {
                                                EventManager.WriteMessage(729, "AddFileAccessRightsToProcessName", EventLevel.Information, "AddFileAccessRightsToProcessName " + filterRule.ProcessNameFilterMask + ",fileNameMask:" + fileNameMask + ",accessFlags:" + accessFlag + " succeeded.");
                                            }
                                        }
                                    }
                                }
                                else if (filterRule.ProcessNameFilterMask.Length > 0)
                                {
                                    ...
                                    if (!FilterAPI.AddFileAccessRightsToProcessName((uint)filterRule.ProcessNameFilterMask.Length * 2, filterRule.ProcessNameFilterMask, (uint)fileNameMask.Length * 2, fileNameMask.Trim(), accessFlag))
                                    {
                                        EventManager.WriteMessage(725, "AddFileAccessRightsToProcessName", EventLevel.Error, "AddFileAccessRightsToProcessName " + filterRule.ProcessNameFilterMask + ",fileNameMask:" + fileNameMask + ",accessFlags:" + accessFlag + " failed:" + FilterAPI.GetLastErrorMessage());
                                    }
                                    else
                ...
                foreach (RegistryFilterRule filterRule in registryFilterRules.Values)
                {
                    ...
                    {
                        uint processId = 0;
                        if (uint.TryParse(filterRule.ProcessId, out processId))
                        {
                            if (!FilterAPI.AddRegFilterEntryById(processId, filterRule.AccessFlag, filterRule.RegCallbackClass, filterRule.IsExcludeFilter))
                            {
                                EventManager.WriteMessage(900, "AddRegFilterEntryById", EventLevel.Error, "AddRegFilterEntry failed:" + FilterAPI.GetLastErrorMessage());
                            }
                            else
                            {
                                EventManager.WriteMessage(901, "AddRegFilterEntryById", EventLevel.Information, "AddRegFilterEntry, processId:" + processId + ",RegistryControlFlag:" + filterRule.AccessFlag + " succeeded.");
                            }
                        }
                    }
                    else if (filterRule.ProcessNameFilterMask.Length > 0)
                    {
                        if (!FilterAPI.AddRegFilterEntryByName((uint)filterRule.ProcessNameFilterMask.Length * 2, filterRule.ProcessNameFilterMask, filterRule.AccessFlag, filterRule.RegCallbackClass, filterRule.IsExcludeFilter))
                        {
                            EventManager.WriteMessage(900, "AddRegFilterEntryByName", EventLevel.Error, "AddRegFilterEntry failed:" + FilterAPI.GetLastErrorMessage());
                        }
                        else
                        {
                            EventManager.WriteMessage(901, "AddRegFilterEntryByName", EventLevel.Information, "AddRegFilterEntry, processName:" + filterRule.ProcessNameFilterMask + ",RegistryControlFlag:" + filterRule.AccessFlag + " succeeded.");
                        }
