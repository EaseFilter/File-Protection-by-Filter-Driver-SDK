...
namespace EaseFilter.CommonObjects
{
    ...
    static public class FilterAPI
    {
        ...
        public enum EncryptionMethod : uint
        {
            ...
            /// <summary>
            /// Use the same encryption key from the filter rule for all files, use unique iv key per file, the iv key will be attached to the 
            /// encyrpted file as a reparse point extended attribute, it only supports NTFS, ReFS. You can identify if the file is encrypted by checking the tagdata.
            /// </summary>
            ENCRYPT_FILE_WITH_SAME_KEY_AND_UNIQUE_IV,
            /// <summary>
            ///Use the encryption key and iv from user mode service, your own service can determine if the file can be encrypted or the encrypted can be opened, 
            ///,you also can control how to use encryption key and iv for files, there are no reparse point tag data added to encrypted file.
            ///You can't identify if the file is encrypted or not by API.
            /// </summary>
            ENCRYPT_FILE_WITH_KEY_AND_IV_FROM_SERVICE,
            /// <summary>
            ///Use the encryption key and iv from user mode service, your own service can determine if the file can be encrypted or the encrypted can be opened, 
            ///,you also can control how to use encryption key and iv for files, you can add your own customized tag data to the encrypted file.
            ///You can identify if the file is encrypted by checking the tagdata.
            /// </summary>
        ...
        public enum BooleanConfig : uint
        {
            ...
            /// </summary>
            REQUEST_ENCRYPT_KEY_IV_AND_TAGDATA_FROM_SERVICE = 0x00000020,  
            /// <summary>
        ...
        //UserName format:  domainName(or computerName)\userName.exe
        ...
        [DllImport("FilterAPI.dll", SetLastError = true)]
        public static extern bool AddRegFilterEntryByName(
        ...
        /// <returns>return true if the entry was added successfully</returns>
        [DllImport("FilterAPI.dll", SetLastError = true)]
        public static extern bool AddRegFilterEntryById(
            uint processId,
            uint accessFlag,
            ulong regCallbackClass,
            bool isExcludeFilter);
        ...
        [DllImport("FilterAPI.dll", SetLastError = true)]
        public static extern bool RemoveRegFilterEntryById(
            uint processId);

        [DllImport("FilterAPI.dll", SetLastError = true)]
        public static extern bool RemoveRegFilterEntryByName(
            uint processNameLength,
            [MarshalAs(UnmanagedType.LPWStr)]string processName);
       ...
       //---------------Process filter APIs-----------------------------------------------
        ...
        /// <returns></returns>
        [DllImport("FilterAPI.dll", SetLastError = true)]
        public static extern bool AddProcessFilterEntry( 
        uint processNameMaskLength,
        [MarshalAs(UnmanagedType.LPWStr)]string processNameMask,
        uint controlFlag );

        [DllImport("FilterAPI.dll", SetLastError = true)]
        public static extern bool RemoveProcessFilterEntry(
        uint processNameMaskLength,
        [MarshalAs(UnmanagedType.LPWStr)]string processNameMask);
        ...
        /// <returns></returns>
        [DllImport("FilterAPI.dll", SetLastError = true)]
        public static extern bool AddFileAccessRightsToProcessName(	
        uint processNameMaskLength,
        [MarshalAs(UnmanagedType.LPWStr)]string processNameMask,
        ...
        /// <returns></returns>
        [DllImport("FilterAPI.dll", SetLastError = true)]
        public static extern bool RemoveFileAccessRightsFromProcessName(
        uint processNameMaskLength,
        [MarshalAs(UnmanagedType.LPWStr)]string processNameMask,
        uint fileNameMaskLength,
        [MarshalAs(UnmanagedType.LPWStr)]string fileNameMask);
        ...
        /// <returns></returns>
        [DllImport("FilterAPI.dll", SetLastError = true)]
        public static extern bool AddFileAccessRightsToProcessId(	
        uint processId,
        uint fileNameMaskLength,
        [MarshalAs(UnmanagedType.LPWStr)]string fileNameMask,
        uint AccessFlag );
        ...
        /// <returns></returns>
        [DllImport("FilterAPI.dll", SetLastError = true)]
        public static extern bool RemoveFileAccessRightsFromProcessId(
        uint processId,
        uint fileNameMaskLength,
        [MarshalAs(UnmanagedType.LPWStr)]string fileNameMask);
