...
namespace FileProtector
{
    public partial class ProtectorForm : Form
    {
        ...
        private void Test()
        {
            ...
            IntPtr unmanagedPointer = Marshal.AllocHGlobal(destination.Length);
            Marshal.Copy(destination, 0, unmanagedPointer, destination.Length);
            // Call unmanaged code
            FilterAPI.MessageSendData messageSend = new FilterAPI.MessageSendData();// (FilterAPI.MessageSendData)Marshal.PtrToStructure(unmanagedPointer, typeof(FilterAPI.MessageSendData));

            messageSend.DataBuffer = new byte[65536];
            messageSend.DataBufferLength = 65536;

         //   filterMessage.AddMessage(messageSend);
         //   RegistryHandler.AuthorizeRegistryAccess(messageSend, unmanagedPointer);
        ...
        private void getEncryptedFileIVTagToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            if (inputForm.ShowDialog() == System.Windows.Forms.DialogResult.OK)
            {
                ...
                if (FilterAPI.GetIVTag(fileName, ref iv,ref accessFlags, out lastError))
                {
                    MessageBoxHelper.PrepToCenterMessageBoxOnForm(this);
